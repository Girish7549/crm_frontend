...
import CreateSaleActivationsModal from './CreateSaleActivationsModal.jsx'
...
const SaleActivations = () => {
  ...
  const get_all__emp_activations = `${baseUrl}/activations/support/${userID?.id}`
  const update_activation = `${baseUrl}/activations`
  const get_all_sale_pending = `${baseUrl}/sales/team-pending/${userID.teamId}`
  ...
  async function getAllSaleCustomer() {
    try {
      const req = await fetch(get_all_sale_pending)
  ...
  async function getAllEmpActivations(page = 1) {
    try {
      const req = await fetch(`${get_all__emp_activations}?page=${page}`)
      const res = await req.json()
  ...
  useEffect(() => {
    socket.on('activation-updated', (data) => {
  ...
  return (
    <>
          ...
          <div>
            ...
            >
              ...
              {columnKeys.includes('saleItemsSummary') && (
                <Column key="saleItemsSummary" width={300} fixed fullText>
      ...
      <CreateSaleActivationsModal
        open={modalOpen}
        handleClose={() => setModalOpen(false)}
        data={data}
        getAllEmpActivations={getAllEmpActivations}
        isEdit={isEdit}
...
export default SaleActivations
